...
class TestBuildSql:
    ...
    def test_multiple_fragments(self):
        ...
        assert "SELECT * INTO #temp_a" in result
        assert "SELECT * INTO #temp_b" in result
        assert "SELECT * INTO #temp_c" in result
        # References in bodies are rewritten to #temp_x
        assert "FROM #temp_a" in result
        assert "FROM #temp_b" in result
        # Last temp table is the terminal SELECT target
    ...
    def test_indentation_applied(self):
        ...
        # "FROM raw" appears inside the SELECT INTO subquery body — must be indented.
